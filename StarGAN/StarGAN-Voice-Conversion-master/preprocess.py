...
def get_spk_world_feats(spk_fold_path, mc_dir_train, mc_dir_test, sample_rate=16000, test_size = 0.1, prep_train = True, prep_test = True, train_size = 360):
	...
	if prep_train:
		f0s = []
		coded_sps = []
		print(f"Preparing the stats train file for {spk_name}...")
		for wav_file in tqdm(train_paths):
			try:
				...
				print(f"TrainStats: Something went wrong with {wav_file}")
				continue
		...
		print(f"Preparing the train files for {spk_name}...")	
		for wav_file in tqdm(train_paths):
			try:
				...
				print(f"Train: Something went wrong with {wav_file}")
				continue
	...
	if prep_test:
		...
		print(f"Preparing the test files for {spk_name}...")
		for wav_file in tqdm(test_paths):
			try:
				...
				print(f"Test: Something went wrong with {wav_file}")
				continue
	return 0
...
if __name__ == '__main__':
	...
	for spk in speaker_used:
		...
		futures.append(executor.submit(partial(get_spk_world_feats, spk_path, mc_dir_train, mc_dir_test, sample_rate, test_size, prep_train, prep_test, train_size)))
	result_list = [future.result() for future in tqdm(futures)]
	print(result_list)
	sys.exit(0)
